...
export default function Home() {
  ...
  useGSAP(() => {
    const stops = logoRef.current?.querySelectorAll('stop');

    itemsRef.current.forEach((item, i) => {
      if (!item || !stops) return;
      ...
      Draggable.create(item, {
        ...
        onDrag() {
          if (!gsap.isTweening(logoRef.current)) {
            if (Draggable.hitTest(item, logoRef.current, 12)) {
              gsap.to(stops, {
                attr: {
                  'stop-color': (_, target) =>
                    itemColor[Array.from(stops).indexOf(target)],
                },
              });
            }
          }
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-[#0e100f] text-[rgb(255,252,225)] font-sans">
      <svg
        ref={logoRef}
      ...
      <div className="text-center mt-12">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 mt-4">
          {[
            'https://assets.codepen.io/16327/flair-11.png',
            'https://assets.codepen.io/16327/flair-3.png',
            'https://assets.codepen.io/16327/flair-2.png',
            'https://assets.codepen.io/16327/flair-17.png',
          ].map((src, i) => (
            <img
              key={i}
              ref={(el) => {
              ...
              src={src}
              alt=""
              className="w-[20%] max-w-[150px] relative cursor-grab"
            />
          ))}
        </div>
      </div>
